...
export class BracketParser {
  ...
  public constructor(
    ...
    this.startSize = this.bracket.start.length;
    this.endSize = this.bracket.end.length;
    this.longest = Math.max(this.startSize, this.endSize);
    this.rgba = rgba;
  }
  ...
  private lookBackwardForBracket(pos: number): { pos: number; scope: number } {
    ...
    while (pos < text.length-this.longest && pos >= 0) {
      pos = this.commentParser.getNextStartOffset(pos);

      if (text.substring(pos, this.startSize) === this.bracket.start) {
        return { pos, scope: 1 };
      }
      if (text.substring(pos, this.endSize) === this.bracket.end) {
        return { pos, scope: -1 };
      }
      pos--;
    }
